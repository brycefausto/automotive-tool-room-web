...
export default function TransactionEditModal({ show, setShow, onClose, transaction, shouldReturn }: TransactionEditModalProps) {
  ...
  const transactItemDtos = useRef<TransactItemDto[]>(transaction.items.map(
    ({ item, returnQuantity, fillStatus, remarks }) => ({ itemId: item._id, returnQuantity, fillStatus: fillStatus?.toString(), remarks })
  ));
                  ...
                  <tr key={i}>
                    ...
                    <td className="px-2">
                      <TextInput
                        ...
                        min={0}
                        max={item.quantity}
                        defaultValue={transactItemDto.returnQuantity}
                        onChange={(e) => transactItemDto.returnQuantity = Math.min(parseInt(e.target.value), item.quantity)}
                      />
                    </td>
                    <td className="px-2 w-[300px]">
                      <TextInput 
                        defaultValue={item.remarks}
                        onChange={(e) => transactItemDto.remarks = e.target.value}
                      />
                    </td>
                  </tr>
                )
